...
msg = MIMEMultipart(html_bytes, 'html')
...
signature = dkim.sign(message=html_bytes, domain="heelas.uk", selector="xmas", identity="xmas@heelas.uk", privkey=key   )
st.write(signature)
...
if pin == st.text_input("PIN"):
    if st.button("Send"):
        with smtplib.SMTP_SSL(smtp_server, smtp_port) as server:
            ...
            server.sendmail(sender_email, recipient_email, subject, signature())
